...
func GetPublishers(c context.Context, start int, limit int) ([]*vo.PublisherVO, error) {
	_, span := otel.Tracer("publisher").Start(c, "db-get-publishers",
		oteltrace.WithAttributes(attribute.String("start", strconv.Itoa(start)),
			attribute.String("limit", strconv.Itoa(limit))))
	models, err := database.Query[models.Publisher]("publishers", bson.D{}, "_id", start, limit)
	span.End()
	if err != nil {
